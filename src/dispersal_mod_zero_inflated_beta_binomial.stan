...
parameters {
  ...
  real <lower = 0> delta; // Auto-infection
  ...
  real <lower = 0> phi; // Nuisance Variance Parameter
}
...
model {
  ...
  for (i in 1 : N_total) {
    ...
    if (y[i] == 0) {
      target += log_mix(pi, 0, beta_binomial_lpmf(0 | n[i], a_i, b_i));
    } else {
      target += log1m(pi) + beta_binomial_lpmf(y[i] | n[i], a_i, b_i);
    }
  }
}
...
generated quantities {
  ...
  for (i in 1 : N_total) {
    ...
    if (bernoulli_rng(pi)) {
        ...
        y_rep[i] = beta_binomial_rng(n[i], a_i, b_i);
    }
  }
}
