...
EDGE_PERCENTILE = 95             # used if mode == "percentile"
...
# Loop through each model and month.
for model in models:
    model_path = os.path.join(results_dir, model)
    months = sorted([d for d in os.listdir(model_path) if os.path.isdir(os.path.join(model_path, d))])
    
    for month in months:
        fit_dir = os.path.join(model_path, month)
        csv_files = glob.glob(os.path.join(fit_dir, "*.csv"))
        
        # Skip if no MCMC output files exist.
        if not csv_files:
            continue
            
        print(f"\n=========================================")
        print(f"Generating network plots for Model: {model}, Month: {month}")
        print("=========================================")
        ...
        try:
            ...
            # Loop through years and generate network visualization plots.
            for year in years:
                ...
                # Print diagnostics.
                print(f"  [{year}] adj_matrix: nonzero = {np.count_nonzero(adj_matrix)}, max = {adj_matrix.max():.4g}")
                print(f"  [{year}] G: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges")
                print(f"  [{year}] G_sub: {G_sub.number_of_nodes()} nodes, {G_sub.number_of_edges()} edges")
                ...
                # Set axes limits, titles, and layout.
                ax.set_aspect('equal')
                ...
                # Save figure to file.
                output_dir = f"output/figures/spatial_network/{model}/{month}"
                os.makedirs(output_dir, exist_ok=True)
                plot_path = os.path.join(output_dir, f"spatial_network_{year}.png")
                plt.savefig(plot_path, dpi=300, bbox_inches='tight')
                plt.close(fig)
                print(f"  Saved plot to {plot_path}")

        except Exception as e:
            print(f"Error processing model {model} for month {month}: {e}")
            plt.close('all')